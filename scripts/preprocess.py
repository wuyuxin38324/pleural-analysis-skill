...
def dicom_to_nifti(dicom_path, output_path):
    ...
    if not series_id:
        raise ValueError(f"未找到DICOM序列: {dicom_path}")
...
def preprocess_pipeline(dicom_dir, output_dir, spacing=[2.94227, 2.5631666, 2.5631666]):
    ...
    完整预处理流程
    Args:
        dicom_dir: DICOM文件目录 (应包含CT和PET子目录)
        output_dir: 输出目录
        spacing: 目标spacing
    """
    ...
    if not ct_dir or not pet_dir:
        raise ValueError(f"未找到CT或PET目录: {dicom_dir}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="预处理DICOM序列为NIfTI格式")
    parser.add_argument("dicom_dir", help="DICOM目录路径")
